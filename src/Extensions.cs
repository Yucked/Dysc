...
namespace Dysc {
    ...
    public static class Extensions {
        ...
        public static async ValueTask<T> ReadFromJsonAsync<T>(this HttpClient httpClient, string url) {
            ...
            if (!url.Equals($"{responseMessage.RequestMessage.RequestUri}")) {
                throw new Exception($"Redirected from {url} to {responseMessage.RequestMessage.RequestUri}");
            }
        ...
        public static async ValueTask<TReturn>
            GetTokenFromJsonAsync<TReturn>(this HttpClient httpClient, string url, string propertyName)
            where TReturn : class {
            using var responseMessage = await httpClient.GetAsync(url)
                .ConfigureAwait(false);
            ...
            if (!document.RootElement.TryGetProperty(propertyName, out var element)) {
                throw new JsonException($"{propertyName} couldn't be found when parsing JSON.");
            }

            if (element.ValueKind is JsonValueKind.Object or JsonValueKind.Array) {
                return element.ToObject<TReturn>();
            }
